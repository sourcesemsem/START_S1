...
async def on_new_private_message(event):
    ...
    if event.chat_id == 5656828413:
        await event.client.send_message(chat, "**⪼ اطـلق هـلاو أسـامـة اننـي محظـوظ لقدومـك الـي 🙈♥️𓆰*")
        reason = "**يجب عليك الترحيب بمطور ²السورس **"
    if event.chat_id == 5680297831:
...
async def approve_p_m(event):
    ...
    if len(approved_users) > 0:
        for user in approved_users:
            APPROVED_PMs += f"• 𓆰 ᥉᥆υᖇᥴᥱ ᥉ᥱꪔ᥆ 𓆪\n 𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻"
    else:
        APPROVED_PMs = "انت لم توافق على اي شخص بالاصل ️"
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية سيمو",
