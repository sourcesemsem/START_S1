...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 5656828413:
        return await edit_or_reply(mention, f"**- لكك دي هذا المطور أسامة وعلى راسك**")
    if user.id == 5680297831:
        return await edit_or_reply(mention, f"**- لكك دي هذه المطورة سمير وعلى راسك**")
    if user.id == 5508855150:
